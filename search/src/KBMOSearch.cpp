...
// RESULTS_PER_PIXEL gets used in search, gpuSearch, and gpuSearchFilter
//
// gpu and gpuFilter both call the function, search. search calls the function
// gpuSearch and gpuSearchFilter, which calls deviceSearch or deviceSearchFilter.
//
// so if we accept a variable USER_RESULTS_PER_PIXEL to both gpu and gpuFilter,
// we can pass that variable to search, and then on to gpuSearch/gpuSearchFilter.
//
void KBMOSearch::gpu(
...
std::vector<RawImage> KBMOSearch::summedMaskedStamps(std::vector<trajectory> t_array, int radius)
{
    int numResults = t_array.size();
    int dim = radius*2+1;
    ...
    for (int s = 0; s < numResults; ++s)
    {
        ...
        for (int x = 0; x < dim; ++x)
        {
            ...
            if (count> 0.0)
            {
                singleStamp.addToPixel(static_cast<int>(x),static_cast<int>(y), sum/count);
            } else {
              singleStamp.addToPixel(static_cast<int>(x),static_cast<int>(y), 0.0);
            }
