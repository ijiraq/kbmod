...
PointSpreadFunc::PointSpreadFunc(float stdev) {
	...
	for (int ii = 0; ii < dim; ++ii) {
		for (int jj = 0; jj < dim; ++jj) {
			...
			//- simpleGauss[abs(radius - jj)] * simpleGauss[abs(radius - jj)] * 0.25; // vertical stripe
			//- simpleGauss[abs(radius - jj)] * simpleGauss[abs(radius - jj)] * 0.2; //horizontal stripe
			// the above line adds in a negative vertical
			// gaussian stripe of the same width as the gaussian
			// adopt a -0.25 depth for now, need to pass this variable to the user
...
PointSpreadFunc::PointSpreadFunc(PointSpreadFunc& other)
{
	...
	calcSum();  // Wes wonders why this is calculated twice. It is already done as the last step of the kernel creation
}
